...
export default function StatusDashboard() {
  ...
  // ---- Fetch dashboard lists from backend (refactored so we can reuse for refresh) ----
  useEffect(() => {
    let mounted = true;
    ...
    // 🔁 Re-fetch when page regains focus / becomes visible (keeps status in sync after actions elsewhere)
    const onFocus = () => fetchLists();
    const onVisibility = () => {
      if (document.visibilityState === 'visible') fetchLists();
    };

    // 🔁 Optional: listen for cross-tab or in-app signals (if other pages set this key after status change)
    const onStorage = (e: StorageEvent) => {
      if (e.key === 'candidate_status_changed') fetchLists();
    };

...
module.exports = {
  ...
  theme: {
    ...
    extend: {
      ...
      // Typography families are wired to CSS variables set by next/font (see layout.tsx)
      fontFamily: {
        sans: ["var(--font-space-grotesk)", "ui-sans-serif", "system-ui"],
        display: ["var(--font-orbitron)", "var(--font-space-grotesk)", "ui-sans-serif"],
      ...
      backgroundImage: {
        ...
        "luxe-conic": "conic-gradient(from 180deg at 50% 50%, hsl(var(--g1)), hsl(var(--g2)), hsl(var(--g3)), hsl(var(--g1)))",
      },

      boxShadow: {
        soft: "0 8px 30px rgba(0,0,0,0.28)",
        lux: "0 1px 0 rgba(255,255,255,0.08) inset, 0 20px 50px rgba(0,0,0,0.55)",
        glow: "0 0 0 1px rgba(255,255,255,0.05), 0 0 24px hsl(var(--primary) / 0.55)",
        neon: "0 0 12px hsl(var(--accent)/0.8), 0 0 28px hsl(var(--secondary)/0.65)",
        "elev-1": "0 2px 8px rgba(0,0,0,0.45), 0 1px 1px rgba(255,255,255,0.04) inset",
        "elev-2": "0 14px 40px rgba(0,0,0,0.55)",
        "inner-highlight": "inset 0 1px 0 rgba(255,255,255,0.08)",
      },

      borderRadius: { xl: "0.9rem", "2xl": "1.25rem", "3xl": "1.75rem", full: "9999px" },
      ...
      keyframes: {
        ...
        "pulse-neon": {
          "0%,100%": { filter: "drop-shadow(0 0 0 hsl(var(--accent)/0.0))" },
          "50%": { filter: "drop-shadow(0 0 18px hsl(var(--accent)/0.9))" },
        },
        "aurora-shift": {
        ...
        "blur-in": { from: { filter: "blur(12px)", opacity: 0.2 }, to: { filter: "blur(0)", opacity: 1 } },
        "rise-in": { from: { transform: "translateY(16px)", opacity: 0 }, to: { transform: "translateY(0)", opacity: 1 } },
        shimmer: {
  ...
  plugins: [
    ...
    // You may also add `tailwindcss-animate` if already installed.
  ],
};
